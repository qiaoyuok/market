...
<div class="cf">
		<div class="fl">
            <a class="btn" href="<?php echo U('Goods/add');?>">新 增</a>
            <!-- <button class="btn ajax-post" url="<?php echo U('Device/changeStatus',array('method'=>'resumedevice'));?>" target-form="ids">启 用</button> -->
            <button class="btn ajax-post" url="<?php echo U('Goods/changeStatus',array('method'=>'deletegoods'));?>" target-form="ids">下架</button>
		...
		<div class="search-form fr cf">
			<div class="sleft">
				...
				<a class="sch-btn" href="javascript:;" id="search" url="<?php echo U('out');?>"><i class="btn-search"></i></a>
			</div>
		</div>
    </div>
    <!-- 数据列表 -->
    ...
    <tbody>
		<?php if(!empty($list)): if(is_array($list)): $index = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($index % 2 );++$index;?><tr>
            <td><input class="ids" type="checkbox" name="id[]" value="<?php echo ($vo["goods_id"]); ?>" /></td>
			...
			<td><?php if(($vo["status"]) == "-1"): ?><a href="<?php echo U('Goods/changeStatus?method=replaygoods&id='.$vo['goods_id']);?>" class="confirm ajax-get">上架</a><?php endif; ?>
				<a href="<?php echo U('Goods/editAction?id='.$vo['goods_id']);?>">编辑</a>
				<a href="<?php echo U('Goods/changeStatus?method=onlydelete&id='.$vo['goods_id']);?>" class="confirm ajax-get">删除</a>
                </td>
    ...
    //导航高亮
    highlight_subnav('<?php echo U('Goods/out');?>');

   $("#cate_name").change(function(e){
    	console.log(e.currentTarget.value);
    	cate_id = e.currentTarget.value;
    	window.open("/index.php?s=/Admin/Goods/out/cate_id/"+cate_id,"_self");
    })
	</script>
