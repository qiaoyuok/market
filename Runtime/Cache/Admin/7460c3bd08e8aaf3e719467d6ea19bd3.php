...
<body>
    ...
    <tbody>
		<?php if(!empty($list)): if(is_array($list)): $index = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($index % 2 );++$index;?><tr>
			...
			<td><?php if(($vo["status"]) == "1"): ?><a href="<?php echo U('Goods/changeStatus?method=deletegoods&id='.$vo['goods_id']);?>" class="confirm ajax-get">下架</a><?php endif; ?>
				<!-- <a href="<?php echo U('Device/changeStatus?method=editdevice&id='.$vo['id']);?>" class="confirm ajax-get">查看</a> -->
    ...
    $("#cate_name").change(function(e){
    	...
    	window.open("/index.php?s=/Admin/Goods/index/cate_id/"+cate_id,"_self");
    })
	</script>
