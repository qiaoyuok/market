...
<body>
    ...
    <form action="<?php echo U(addlock);?>" method="post" class="form-horizontal">
        <!-- <div class="form-item">
        ...
        <div class="form-item">
            <label class="item-label">锁板id号<span class="check-tips">（锁板唯一标志）</span></label>
            <div class="controls">
                <input type="text" class="text input-large" name="lock_id" value="">
            </div>
        </div>
        <div class="form-item">
            ...
            <div class="controls">
                <select name="shop_id">
                   <?php if($shoplists != null): if(is_array($shoplists)): $i = 0; $__LIST__ = $shoplists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["shop_id"]); ?>"><?php echo ($vo["shop_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                    <?php else: ?><option>请先添加店铺</option><?php endif; ?>
                </select>
            </div>
        </div>
