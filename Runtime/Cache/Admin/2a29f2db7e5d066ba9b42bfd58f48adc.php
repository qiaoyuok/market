...
<body>
	...
	<div class="cf">
		<div class="fl">
            ...
            <!-- 已认证 ：1；待审核：0；未通过：-1 -->
            <!-- <a class="btn" href="<?php echo U('User/index');?>">管理员列表</a> -->
		...
		<div class="search-form fr cf">
			<div class="sleft">
				...
				<a class="sch-btn" href="javascript:;" id="search" url="<?php echo U('user');?>"><i class="btn-search"></i></a>
			</div>
		</div>
    </div>
    <!-- 数据列表 -->
    ...
    <tbody>
		<?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<td><a href="<?php echo U('User/tocheck?id='.$vo['uid']);?>">查看资料</a>
				<?php if(($vo["status"]) == "1"): ?><a href="<?php echo U('User/changeStatus?method=resumeUser&id='.$vo['uid']);?>" class="ajax-get">通过</a>
					<a href="<?php echo U('User/changeStatus?method=forbidUser&id='.$vo['uid']);?>" class="ajax-get">不通过</a><?php endif; ?>
				<a href="<?php echo U('User/changeStatus?method=deleteuser&id='.$vo['uid']);?>" class="confirm ajax-get">删除</a>
				<?php if($vo["status"] != 4): ?><a href="<?php echo U('User/changeStatus?method=darkhoom&id='.$vo['uid']);?>" class="confirm ajax-get">拉黑</a><?php endif; ?>
                </td>
    ...
    $("#vipstatus").change(function(e){
    	...
    	window.location.href="/index.php?s=/Admin/User/normal/status/"+status;
    })
