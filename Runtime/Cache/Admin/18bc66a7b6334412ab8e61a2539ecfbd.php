...
<body>
	...
	<div class="cf">
		...
		<div class="search-form fl cf">
			<div class="sleft" >
				　姓名：<input type="text" style="float: none;width: 120px;" name="name" class="search-input" value="<?php echo I('name');?>" placeholder="请输入用户姓名">
				
			</div>
			<div class="sleft">
				　设备id：<input type="text" name="device_id" style="float: none;width: 120px;" id="getdevice" class="search-input" value="<?php echo I('device_id');?>" placeholder="请输入设备ID号">
			...
			<?php if(($is_show) == "1"): ?><div class="sleft">
				...
				<div class="sleft">
				　店铺名：<input type="text" name="shop_name"  id="getshop" style="float: none;width: 120px;" class="search-input" value="<?php echo I('shop_name');?>" placeholder="请输入店铺名称">
					<!-- <a class="sch-btn" href="javascript:;" style="float: right;" id="search" url="<?php echo U('index');?>"><i class="btn-search"></i></a> -->
				</div><?php endif; ?>
            
			<div class="sleft">
				订单号：<input type="text" name="sn" style="float: none;width: 120px;" class="search-input" value="<?php echo I('sn');?>" placeholder="请输入订单编号">
			</div>
			<div class="sleft">
        	...
        	<button class="btn" id="search" style="float: left;margin-top: 5px;" url="<?php echo U('index');?>">搜　索</button>
		</div>
    </div>
    ...
    <thead>
		...
		<th class="">订单时间</th>
		<th class="">订单状态</th>
		<?php if(($uid) == "1"): ?><th class="">所属加盟商</th><?php endif; ?>
		<th class="">店铺名称</th>
		<th class="">设备ID号</th>
		<th class="">操作</th>
		</tr>
    ...
    <tbody>
		<?php if(!empty($list)): if(is_array($list)): $index = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($index % 2 );++$index;?><tr>
			...
			<td><?php echo ($vo["allcount"]); ?></td>
			<td><?php echo (date("Y-m-d H:i:s",$vo["add_time"])); ?></td>
			<td><?php if(($vo["status"]) == "1"): ?>已完成<?php else: ?>待支付<?php endif; ?></td>
			<?php if(($uid) == "1"): ?><td class=""><?php echo ($vo["business_name"]); ?></td><?php endif; ?>
			<td><?php echo ($vo["shop_name"]); ?></td>
			<td><?php echo ($vo["device_id"]); ?></td>
			<td>
				<a href="<?php echo U('Order/orderdetail?order_id='.$vo['order_id']);?>">查看</a>
				<a href="<?php echo U('Order/changeStatus?method=deleteorder&id='.$vo['order_id']);?>" class="confirm ajax-get">删除</a>
            </td>
...
$(function(){
	...
	$("#status").change(function(e){
		...
		window.location.href="/index.php?s=/Admin/Order/index/status/"+e.currentTarget.value
	})
...
layui.use('table',function(){
  ...
  table.render({
    elem: '#device'
    ,url:'/index.php?s=/Admin/Order/devicelist'
    ,limit:6
    ,limits:[6]
    ,cols: [[
...
layui.use('table',function(){
  ...
  table.render({
    elem: '#business'
    ,url:'/index.php?s=/Admin/Order/businesslist'
    ,limit:6
    ,limits:[6]
    ,cols: [[
...
layui.use('table',function(){
  ...
  table.render({
    elem: '#shop'
    ,url:'/index.php?s=/Admin/Order/shoplist'
    ,limit:6
    ,limits:[6]
    ,cols: [[
