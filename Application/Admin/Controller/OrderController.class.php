...
class OrderController extends AdminController {
    ...
    public function index(){
        ...
        // 自定义分页连表查询
        // 默认条件
        $where = "o.uid = u.uid and o.shop_id = s.shop_id and s.business_id = b.business_id and o.status>=0";
        ...
        if (!empty($_GET["time-start"])) {
            ...
            $where.=" AND o.add_time >=".strtotime($time_start);
        }

        if (!empty($_GET["time-end"])) {
            $time_end = I("time-end")." 23:59:59";
            $where.=" AND o.add_time <=".strtotime($time_end);
        }
        ...
        if (isset($_GET["status"])) {
            $where.=" AND o.status =".I("status");
        }

        if (!empty($_GET["sn"])) {
            $where.=" AND o.sn like '%".I("sn")."%'";
        }
        
        $page = new \Think\Page($order->table('mk_order o,mk_shop s,mk_business b,mk_user u')->where($where)->count(),10);
        // var_dump($page);
        $list = $order->table('mk_order o,mk_shop s,mk_business b,mk_user u')
                        ->field('o.*,u.name,b.business_name,s.shop_name')
                        ->where($where)
        ...
        foreach ($list as $k => $v) {
            ...
            $list[$k]['goodsdetail'] = json_decode($list[$k]['goodsdetail'],true);
        }
        // var_dump($list);exit;
    ...
    public function orderdetail($order_id=''){
    	...
    	$order = M("order");
    	$orderdetail = $order->where("order_id = ".$order_id)->find();
    	$orderdetail['goodsdetail'] = json_decode($orderdetail['goodsdetail'],true);
