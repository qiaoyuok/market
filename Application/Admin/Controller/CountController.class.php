...
class CountController extends AdminController {
    ...
    public function index(){
        ...
        $finance   =   D('Finance');
        // 自定义分页连表查询
        $page = new \Think\Page($finance->where("status EGT 0")->count(),10);
        $list = $finance->table('mk_device d,mk_shop s,mk_finance f,mk_member m')
                        ->field('f.order_price,m.nickname,d.device_name,s.shop_name,f.add_time')
                        ->where('f.shop_id = d.shop_id AND f.device_id = d.device_id AND f.shop_id = s.shop_id and f.uid = m.uid')
