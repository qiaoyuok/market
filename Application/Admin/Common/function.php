 ...
 *
 */
function int_to_string(&$data,$map=array('status'=>array(1=>'正常',-1=>'删除',0=>'关闭',2=>'未审核',3=>'草稿'))) {
...
function get_list($list,$cate_parent=0,$lev=0){
    $tree = [];
    foreach ($list as $k => $v) {
       if ($v['cate_parent']==$cate_parent) {
            $v['lev'] = $lev;
            $tree[] = $v;
            $tree = array_merge($tree,get_list($list,$v['cate_id'],$lev+1));
       }
    }
    return $tree;
}
...
function cut($str=''){
    ...
    if (mb_strlen($str,'utf-8')>16) {
        ...
        return mb_substr($str, 0,16,'utf-8')."&nbsp;<img style='width:20px;' src='/Public/Admin/images/more.png'/>";
    }else{
        return $str;
    }
}
